...
The frontend uses Vite (Node.js 22.12+). `npm run build` creates the production bundle. The API URL is set at build time by `REACT_APP_BACKEND_URL`; an empty value uses the same host's `/api` route.
