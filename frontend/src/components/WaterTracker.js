...
export const WaterTracker = ({ selectedDate, totalWaterMl, onWaterLogged, streak, expanded = false, onExpand }) => {
    ...
    return <section className="ft-card ft-small-card" data-testid={expanded ? "water-entry" : "water-tracker"}>
      <div className="ft-section-head"><h2><Droplet size={18}/>Water</h2>{!expanded && <button className="ft-icon-button" aria-label="Water options" onClick={onExpand}><Plus size={18}/></button>}</div>
      <div className="ft-stat-number">{(totalWaterMl/1000).toFixed(1)}<span> / 3.0 L</span></div>
      <div className="ft-track ft-water" role="progressbar" aria-label="Water intake" aria-valuenow={Math.min(Math.round(totalWaterMl/3000*100),100)} aria-valuemin={0} aria-valuemax={100}><span style={{width:`${Math.min(totalWaterMl/3000*100,100)}%`}}/></div>
