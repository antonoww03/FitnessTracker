...
const handlers={},stored=new Map();let assets=[],offline=false,claimed=false;
...
vm.runInNewContext(fs.readFileSync(path.join(__dirname,'../public/service-worker.js'),'utf8'),{URL,self:{location:{origin:'https://test.invalid'},clients:{claim:async()=>{claimed=true}},addEventListener:(name,fn)=>{handlers[name]=fn}},caches:{open:async()=>cache,keys:async()=>[],match:async k=>stored.get(k)},fetch:async()=>{if(offline)throw new Error('offline');return response}});
(async()=>{
...
console.log('PASS: PWA asset precache, activation, API bypass and offline navigation fallback.');
